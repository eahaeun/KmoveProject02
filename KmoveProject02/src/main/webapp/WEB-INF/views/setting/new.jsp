		...
		<form action="/setting/sikyuCreate" method="post" id="sikyuForm">
				...
				<tbody>
					<c:forEach items="${sikyuList}" var="sikyu">
						<tr class="sikyu-row" data-sikyu-km="${sikyu.sikyu_km}">
							...
							<td><c:out value="${sikyu.kazei_active}" /></td>
							<td><c:out value="${sikyu.hikazei_limit}" /></td>
			...
			<table>
				<tbody id="sikyuTableBody">
					<tr>
						<th>支給項目</th>
						<td><input type="text" id="sikyu_km" name="sikyu_km" value="" /></td>
					</tr>

					<tr>
						<th>과세여부</th>
						<td><input type="radio" id="kazei_active_O"
							name="kazei_active" value="O" />使用 <input type="radio"
							id="kazei_active_X" name="kazei_active" value="X" />使用しない</td>
					</tr>

					<tr>
						<th>비과세명</th>
						<td><input type="text" id="hikazei_nm" name="hikazei_nm"
							value="" /></td>
					</tr>

					<tr>
						<th>비과세한도액</th>
						<td><input type="text" id="hikazei_limit"
							name="hikazei_limit" value="" /></td>
					</tr>
		...
		<form action="/setting/kojyoCreate" method="post">
			...
			<table>
				<tbody id="kojyoTableBody">
					<tr>
						<th>控除項目</th>
						<td><input type="text" id="kojyo_km" name="kojyo_km" value="" /></td>
					</tr>
					<tr>
						<th>端数単位</th>
...
<script>
			...
			function() {
				...
				$.ajax({
					...
					success : function(response) {
						...
						console.log(response.sikyu_km, response.hansu_tani,
								response.ikkatsu_pay, response.sikyu_active);

						$('#sikyu_km').val(response.sikyu_km);
						$(
								'input[name=kazei_active][value='
										+ response.kazei_active + ']').prop(
								'checked', true);
	...
	$('#sikyuUpdateButton').click(function() {
		...
		var sikyu_tani = $('#hikazei_nm').val();
		var sikyu_tani = $('#hikazei_limit').val();
